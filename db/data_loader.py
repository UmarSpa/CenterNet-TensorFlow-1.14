...
from .data_sampler_tf import tf_dataset_init
from db.coco.PythonAPI.pycocotools.coco import COCO
from db.coco.PythonAPI.pycocotools.cocoeval import COCOeval

def init_db(db_cfg, sys_cfg):
    training_db = dataset(db_cfg, db_cfg['train_split'])
    validation_db = dataset(db_cfg, db_cfg['val_split'])
    # train_cfg = training_db.configs
    val_cfg = validation_db.configs

    print("Validation db")
    valid_tf_dataset = tf_dataset_init(validation_db, sys_cfg)

    print("Training db")
    train_tf_dataset = tf_dataset_init(training_db, sys_cfg)

    return train_tf_dataset, valid_tf_dataset, val_cfg, validation_db

class dataset(DETECTION):
    def __init__(self, db_config, split):
        super(dataset, self).__init__(db_config)
        self.data_dir   = db_config['data_dir']
        self.cache_dir  = db_config['cache_dir']
        print(self.data_dir)
        ...
        self._coco_dir = os.path.join(self.data_dir, "coco")
        ...
        self._data = "coco"
        if db_config['backbone_type'] == 'hourglass52':
            self._mean = np.array([0.40789654, 0.44719302, 0.47026115], dtype=np.float32)
            self._std  = np.array([0.28863828, 0.27408164, 0.27809835], dtype=np.float32)
        elif db_config['backbone_type'] == 'resnet50' or db_config['backbone_type'] == 'resnet18':
        ...
        self._cache_file = os.path.join(self.cache_dir, "coco_{}.pkl".format(self._dataset))
        self._load_data()
    ...
    def evaluate_human(self, result_json, image_ids, gt_json=None, TB_obj=None, TB_iter=0):
        if self._split == "testdev":
            return None
